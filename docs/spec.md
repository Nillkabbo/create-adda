...
- Every pushed `v*` tag gets a GitHub Release once CI passes (`.github/workflows/test.yml`, job `release`). `scripts/release-notes.mjs` builds the notes from the Conventional Commit subjects since the previous tag (`feat` → New, `fix` → Fixes, everything else left out). A changed `src/rules.md` opens the notes with how to refresh installed rules (`--prefs`, and `claude plugin update` for the plugin); changed everyday texts get their own callout, without a link to the site while it is unlisted. This is the only update notification: the tool itself never checks the network.
- Rule wording changes ship only when the eval shows no regression. `npm version` enforces this through `scripts/eval-gate.mjs` (`preversion`): when `src/rules.md` or `src/profiles/everyday/` changed since the last `v*` tag, it runs that suite with 3 runs (developer on haiku, everyday on sonnet), and `eval/run.mjs` exits non-zero unless every scenario passed on most of its runs and none errored (`eval/verdict.mjs`). Majority, not unanimity, because haiku sometimes misses the Rule on a short reply. The everyday suite runs on sonnet because everyday users are on the web apps' default models, and haiku mixes Bengali script into Banglish replies whatever the wording (#11); the site already tells users that small models handle Latin-script Bangla less well. `ADDA_SKIP_EVAL=1` skips the gate. Offline, `test/rules-lint.test.js` guards the word budget, Bengali-script use, and the example count.
